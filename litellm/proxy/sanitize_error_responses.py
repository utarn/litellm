...
def get_sanitized_error_message(
    exception: Exception, exception_type: str = "APIConnectionError"
) -> str:
    """
...
def sanitize_error_response(
    ...
    if status_code is None:
        if hasattr(exception, "status_code") and isinstance(
            getattr(exception, "status_code"), int
        ):
    ...
    if preserve_auth_errors and (
        "auth" in str(exception_type).lower()
        or status_code in [401, 403]
        or "authentication" in str(exception).lower()
        or "authorization" in str(exception).lower()
    ):
    ...
    sanitized_response = {
        "error": {"message": message, "type": exception_type, "code": status_code}
    }
...
def create_sanitized_proxy_exception(
    ...
    try:
        ...
        sanitized = sanitize_error_response(
            original_exception, preserve_auth_errors=preserve_auth_errors
        )
