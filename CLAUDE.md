...
- **Keep schema files in sync.** Apply schema changes to all `schema.prisma` copies (`schema.prisma`, `litellm/proxy/`, `litellm-proxy-extras/`) with a migration under `litellm-proxy-extras/litellm_proxy_extras/migrations/`. At v1.89.5 the standalone `litellm-js/spend-logs/schema.prisma` was consolidated away and no longer exists; do not look for it.
...
3. **Update litellm-proxy-extras** — at v1.89.5 this is a monorepo workspace member (`members = ["enterprise", "litellm-proxy-extras"]` in `pyproject.toml`), not a standalone PyPI dependency. Its migration directory lives in the worktree at `litellm-proxy-extras/litellm_proxy_extras/migrations/`; add the migration file there and it is picked up by `prisma migrate deploy` on startup. Because the package is built from the workspace into the image (not pip-installed from PyPI), a migration lands by rebuilding the image rather than by bumping a published package version.

### Production deployment (modelharbor)
The production proxy runs on host `modelharbor` (SSH `root@modelharbor`, public-key auth; node reports as `utarn-blockchain-node`). Plain **docker compose** (not Swarm) at `/root/mh/litellm`.

- **Main service**: container `litellm_main`, image `registry2.zarimpun.com/utarn/litellm:latest`. Inside the image litellm runs from source at `/app/litellm` (not site-packages). `config.yaml` is bind-mounted (`./config.yaml:/app/config.yaml`) — so config-only changes need just an edit + restart; **code changes require a new image**.
- **Build pipeline**: `.github/workflows/docker-build.yml` triggers on push to branch `vv1.89.5`, builds `./Dockerfile` and pushes `registry2.zarimpun.com/utarn/litellm:latest` and `registry2.zarimpun.com/utarn/litellm:vv1.89.5` (plus a `YYYYMMDD-HHMM` tag). `origin` is GitHub (`github.com/utarn/litellm`); `gh` is auth'd as `utarn`. At v1.89.5 the image is built with `litellm-proxy-extras` and `enterprise` as in-tree workspace members (not pip-installed from PyPI), so a code or migration change in either lands simply by rebuilding the image.
- **Deploy a code change** (full path): commit → `git push origin vv1.89.5` → watch `gh run watch` until `docker-build.yml` succeeds → on modelharbor: `cd /root/mh/litellm && docker compose pull litellm && docker compose up -d litellm`. Verify with `docker exec litellm_main python -c "import litellm; print(litellm.__file__)"`.
- **Stale script**: `/root/mh/litellm/restart-litellm.sh` uses `docker service update --force` (Swarm) but the node is **not** a Swarm manager — use `docker compose` instead.
- Other litellm stacks on the same host (`litellm_nti-*`, `litellm_local_package-*`) are unrelated deployments — do not touch them.

### Anthropic `/v1/messages` passthrough custom pricing
Custom per-token pricing set in a model's `model_info` (or `litellm_params`) is registered under the deployment's auto-generated `model_id` (a UUID); the provider-prefixed model name (e.g. `anthropic/glm-5.2`) is deliberately registered **cost-stripped** (see `router._create_deployment`). So `get_model_info("anthropic/<model>")` showing `input_cost_per_token: 0` is by design — the real price lives under the UUID.

- At v1.89.5 the custom-pricing resolution is **upstream** (not a fork patch). The Anthropic passthrough logging handler (`_create_anthropic_response_logging_payload`) calls `logging_obj.get_router_model_id()`, which reads `model_info["id"]` from the logging metadata (`metadata` / `litellm_metadata`), then passes `custom_pricing` (from `use_custom_pricing_for_model`) and that `router_model_id` into `litellm.completion_cost` (see `_select_model_name_for_cost_calc`). The metadata `model_info["id"]` is the reliable source for `/v1/messages` passthrough: the response is built with empty `litellm_params`, and the upstream rewrites `model` (e.g. the alias `glm-max-claudecode` arrives as `glm-5.2`), so `request_body.model` and `_hidden_params` are not reliable and a router lookup by alias would fail. This is the same source `SpendLogs.model_id` uses.
- The scalar `response_cost` is persisted to `kwargs["response_cost"]` and `logging_obj.model_call_details["response_cost"]`. Whether the per-component breakdown is also populated for this fork's custom-priced glm model is exactly what the end-to-end passthrough-cost verification checks; treat that verification as the source of truth rather than a carried patch.
- **Fallback policy: fresh minimal patch, never a replay.** If end-to-end verification ever shows `response_cost == 0` (or an all-zero breakdown) for a custom-priced passthrough model on the current base, write a fresh minimal patch against that base. Do not replay the obsolete six-commit passthrough-cost series; those were merged upstream and carrying them would conflict with the richer upstream implementation.
- To verify a custom-priced model is costed: in the UI request detail, both the Metrics "Cost" and the Cost Breakdown "Total"/components must be `> 0` and the SpendLogs `spend == metadata.cost_breakdown.total_cost`. (`model_map_value.input_cost_per_token` stripped→0 is normal; the real price lives under the UUID.)
