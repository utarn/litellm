...
- **Build pipeline**: `.github/workflows/docker-build.yml` triggers on push to branch `vv1.82.3-stable.patch.4`, builds `./Dockerfile` and pushes `registry2.zarimpun.com/utarn/litellm:latest` (plus a `YYYYMMDD-HHMM` tag). `origin` is GitHub (`github.com/utarn/litellm`); `gh` is auth'd as `utarn`.
- **Deploy a code change** (full path): commit → `git push origin vv1.82.3-stable.patch.4` → watch `gh run watch` until `docker-build.yml` succeeds → on modelharbor: `cd /root/mh/litellm && docker compose pull litellm && docker compose up -d litellm`. Verify with `docker exec litellm_main python -c "import litellm; print(litellm.__file__)"`.
...
- Cost calc uses the UUID entry **only when** `custom_pricing=True` and a `router_model_id` are passed to `completion_cost` (see `_select_model_name_for_cost_calc`). The Anthropic passthrough logging handler (`_create_anthropic_response_logging_payload`) computes cost with **three** requirements, all of which were missing in the original code and had to be fixed in sequence:
  1. Pass `custom_pricing` + `router_model_id` into `completion_cost` (else it resolves the stripped name → `$0`).
  2. Resolve `router_model_id` from **`model_info["id"]` in the logging metadata** (`metadata`/`litellm_metadata`). The response `_hidden_params` and `request_body.model` are NOT reliable for `/v1/messages` passthrough — the response is built with empty `litellm_params`, and the upstream rewrites `model` (e.g. the alias `glm-max-claudecode` arrives as `glm-5.2`), so a router lookup by `request_body` alias fails. The metadata `model_info["id"]` is the same source `SpendLogs.model_id` uses and is reliably populated.
  3. **Persist** the computed cost onto `result._hidden_params["response_cost"]` AND `logging_obj.model_call_details["response_cost"]` (mirrors the Gemini/Vertex passthrough handlers). Otherwise `_process_hidden_params_and_response_cost` cannot find it and falls through to `_response_cost_calculator`, which recomputes `$0` for passthrough.
  4. **Populate the breakdown** by also passing `litellm_logging_obj=logging_obj` into that same `completion_cost` call. Without it only the scalar `response_cost` is computed; `logging_obj.cost_breakdown` stays empty, so the request-detail "Cost Breakdown" card shows `$0` for every component even though the spend/total is correct. The breakdown is sourced from `logging_obj.cost_breakdown` in `get_standard_logging_object_payload`.
- **`set_cost_breakdown` guards against a `$0` recompute clobber.** A second cost path — `_update_response_metadata` (response-header enrichment) → `_response_cost_calculator` — runs on the same logging object and resolves the cost-stripped name → `$0`. It races with the handler; when it runs *after* the handler it would overwrite the correct breakdown with all-zeros (was ~56% of glm rows: `spend>0, breakdown=0`). `set_cost_breakdown` now keeps an existing non-zero breakdown instead of letting a `$0` call overwrite it, so the stored value is correct regardless of path ordering. Don't remove this guard.
