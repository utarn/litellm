...
- **Keep schema files in sync.** Apply schema changes to all `schema.prisma` copies (`schema.prisma`, `litellm/proxy/`, `litellm-proxy-extras/`) with a migration under `litellm-proxy-extras/litellm_proxy_extras/migrations/`. At v1.89.1 the standalone `litellm-js/spend-logs/schema.prisma` was consolidated away and no longer exists; do not look for it.
...
- **Build pipeline**: `.github/workflows/docker-build.yml` triggers on push to branch `vv1.89.1`, builds `./Dockerfile` and pushes `registry2.zarimpun.com/utarn/litellm:latest` (plus a `YYYYMMDD-HHMM` tag). `origin` is GitHub (`github.com/utarn/litellm`); `gh` is auth'd as `utarn`. At v1.89.1 the image is built with `litellm-proxy-extras` and `enterprise` as in-tree workspace members (not pip-installed from PyPI), so a code or migration change in either lands simply by rebuilding the image.
- **Deploy a code change** (full path): commit → `git push origin vv1.89.1` → watch `gh run watch` until `docker-build.yml` succeeds → on modelharbor: `cd /root/mh/litellm && docker compose pull litellm && docker compose up -d litellm`. Verify with `docker exec litellm_main python -c "import litellm; print(litellm.__file__)"`.
