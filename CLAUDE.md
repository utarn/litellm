...
- A `[PASSTHROUGH_COST_DEBUG]` **WARNING** line is emitted per `/v1/messages` request (model, model_id sources, `custom_pricing`, registry entry costs, token counts, final `response_cost`). It is at WARNING so it shows even with `set_verbose: False`. Grep `PASSTHROUGH_COST_DEBUG` in proxy logs; it is diagnostic and safe to remove once pricing is confirmed (it adds one line per request).
- To verify a custom-priced model is costed: in the UI request detail, `model_map_value.input_cost_per_token` (stripped→0 is normal) vs the spend log `response_cost` (should be > 0).
